...
class EM(autograd.Function):
    ...
    def forward(ctx, inputs, indexes, features, momentum):
        ...
        outputs = inputs.mm(ctx.features.t())                      # 网络所提取的特征和簇平均特征相乘得到outputs
        return outputs
...
class Memory(nn.Module):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2):
        ...
        self.temp = temp
# 这里memorybank features为train总图像数           需要改为聚类数
        self.register_buffer('features', torch.zeros(num_samples, num_features))
        # features--(source centers+tgt features)
        self.register_buffer('labels', torch.zeros(num_samples).long())   # 在刚定义时通过memory.labels 手动初始化
    ...
    def forward(self, inputs, indexes, symmetric=False):
        ...
        targets = self.labels[indexes].clone()     # 得到这个batch的标签targets
        labels = self.labels.clone()  # 16522, whole labels

        # get centroids for each id
        sim = torch.zeros(labels.max() + 1, B).float().cuda()
        # re-arange simi matrix according to labels to find centroids    将所有相同标签的特征相加
        sim.index_add_(0, labels[labels != -1], inputs[:, labels != -1].t().contiguous())

        nums = torch.zeros(labels.max() + 1, 1).float().cuda()
        # get counter  获得同一个标签的图像个数nums
        nums.index_add_(0, labels[labels != -1], torch.ones(labels[labels != -1].shape[0], 1).float().cuda())
        sim /= nums.clone().expand_as(sim)  # compute centroids 相除得到平均特征  B * 类别数
        softMask = torch.zeros(sim.t().shape).cuda()               # 定义onehot标签容器
        softMask.scatter_(1, targets.view(-1, 1), 1)               # 获得onehot标签
        loss = -(softMask * F.log_softmax(sim.t(), dim=1)).sum(1).mean()      # 点乘求和求平均
        loss_sym = 0
        if symmetric:
            loss_sym = -(F.softmax(sim.t(), 1) * F.log_softmax(softMask, dim=1)).sum(1).mean()   # 点乘相加求和求平均
